...
Query createSaveQuery(
    const std::set<FeatureName>& features, const std::string& passpharse, const std::string& sessionToken = {});
Query createRestoreQuery(const std::map<FeatureName, Feature>& restoreData, const std::string& passpharse,
    const std::string& version, const std::string& checksum, const std::string& sessionToken = {});
Query createRestoreListQuery(const std::list<std::map<FeatureName, Feature>>& restoreData,
    const std::string& passpharse, const std::string& version, const std::string& sessionToken = {});
Query createResetQuery(
    const std::set<FeatureName>& features, const std::string& version, const std::string& sessionToken = {});
...
{
    return ((lhs.data() == rhs.data()) && (lhs.version() == rhs.version()));
}
inline bool operator!=(const Feature& lhs, const Feature& rhs)
{
    return !(lhs == rhs);
...
inline bool operator==(const FeatureStatus& lhs, const FeatureStatus& rhs)
{
    return ((lhs.status() == rhs.status()) && (lhs.error() == rhs.error()));
}
inline bool operator!=(const FeatureStatus& lhs, const FeatureStatus& rhs)
{
    return !(lhs == rhs);
...
inline bool operator!=(const SaveQuery& lhs, const SaveQuery& rhs)
{
    return !(lhs == rhs);
}

bool        operator==(const RestoreQuery& lhs, const RestoreQuery& rhs);
inline bool operator!=(const RestoreQuery& lhs, const RestoreQuery& rhs)
{
    return !(lhs == rhs);
}

bool        operator==(const ResetQuery& lhs, const ResetQuery& rhs);
inline bool operator!=(const ResetQuery& lhs, const ResetQuery& rhs)
{
    return !(lhs == rhs);
}

inline bool operator==([[maybe_unused]] const ListFeatureQuery& lhs, [[maybe_unused]] const ListFeatureQuery& rhs)
{
    return true;
}
inline bool operator!=(const ListFeatureQuery& lhs, const ListFeatureQuery& rhs)
{
    return !(lhs == rhs);
}
...
Response createSaveResponse(const std::map<FeatureName, FeatureAndStatus>& mapFeaturesData, const std::string& version,
    const std::string& checksum, const FeatureStatus& featureStatus);
Response createRestoreResponse(const std::map<FeatureName, FeatureStatus>& mapStatus);
Response createRestoreResponse(
    const FeatureStatus& featureStatus, const std::map<FeatureName, FeatureStatus>& mapStatus);
...
Response createListFeatureResponse(const std::map<FeatureName, FeatureDependencies>& mapFeaturesDependencies,
    const std::string& version, const std::string& passphrassDefinition);
Response createListFeatureResponse(const std::map<FeatureName, FeatureDependencies>& mapFeaturesDependencies,
    const std::string& version, const std::string& passphrassDefinition, const std::string& passphrassDescription);
...
// Comparison operators => for tests mostly
inline bool operator==(const FeatureAndStatus& lhs, const FeatureAndStatus& rhs)
{
    return ((lhs.status() == rhs.status()) && (lhs.feature() == rhs.feature()));
}
inline bool operator!=(const FeatureAndStatus& lhs, const FeatureAndStatus& rhs)
{
    return !(lhs == rhs);
...
inline bool operator!=(const FeatureDependencies& lhs, const FeatureDependencies& rhs)
{
    return !(lhs == rhs);
}
...
inline bool operator!=(const SaveResponse& lhs, const SaveResponse& rhs)
{
    return !(lhs == rhs);
}

bool        operator==(const RestoreResponse& lhs, const RestoreResponse& rhs);
inline bool operator!=(const RestoreResponse& lhs, const RestoreResponse& rhs)
{
    return !(lhs == rhs);
}

bool        operator==(const ResetResponse& lhs, const ResetResponse& rhs);
inline bool operator!=(const ResetResponse& lhs, const ResetResponse& rhs)
{
    return !(lhs == rhs);
}

bool        operator==(const ListFeatureResponse& lhs, const ListFeatureResponse& rhs);
inline bool operator!=(const ListFeatureResponse& lhs, const ListFeatureResponse& rhs)
{
    return !(lhs == rhs);
}
